...
$sql = "SELECT P.maker, L.model, L.speed, L.ram, L.hd, L.screen From Laptops L INNER JOIN Products P ON L.model = P.model WHERE L.speed >= ".$speed." AND L.ram >= " .$ram." AND L.screen >= ".$screen." AND L.hd >= ".$hardDisk.";";
...
if ($row_number > 0 ) {
	...
	while($row = $result->fetch_assoc() ) {
       echo
        ...
        "<div class = \"result\"><img class = \"result_image\" src=\"images/lap2.jpg\"
		alt=\"computer photo\" height = 180px; width=200px;>
		<p>Maker: {$row['maker']}</p>
		<p>Model: {$row['model']}</p> "."<p>Speed: {$row['speed']}</p><p>Ram: {$row['ram']}</p>"."<p>HardDisk: {$row['hd']}</p>
   <p>Screen: {$row['screen']}</p></div>";
