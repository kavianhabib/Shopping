...
<!-- ******************** Photo Viewer **************** -->
    ...
    <form method="post" action="budget.php" class="search_form">
      <label class="moveit">Budget:</label>
            <input type="number" class="lowest" name="budget" min="0" placeholder="Budget">
    <label>Lowest Speed:</label>
            <input type="number" class="highest" name ="speed" min="0" placeholder="Lowest speed">
...
$sql = "SELECT DISTINCT P.model as m1,C.model as m2,C.speed FROM Printers P,PCs C WHERE P.price + C.price <= " .$budget . " AND C.speed >= " .$speed . ";";
...
if ($row_number > 0) {
	...
	while($row = $result->fetch_assoc()) {
	   echo
        ...
        "<div class = \"result\"><img class = \"result_image\" src=\"images/dp.jpg\"
		alt=\"computer photo\" height = 200px; width=200px;> 
		<p>Printer Model: {$row['m1']}</p>"."<p>Computer Model: {$row['m2']}</p> "."<p>Speed: {$row['speed']}</p>  "."</div>";
